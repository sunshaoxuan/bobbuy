...
2.  **IM 接口对接 (调查建议)**：
    ...
    -   **同步策略**：当前实现以 REST 接口持久化消息，再通过轮询刷新与页面事件触发 AI 结果回流；Webhook / WebSocket 仍是后续演进方向。
...
-   **前端响应（当前）**：使用 REST + 轮询刷新承载“边发边看”的联调闭环，并在 UI 中明确私聊 / 订单聊天 / Trip 聊天三种上下文边界。
-   **前端响应（后续）**：WebSocket 仍是后续可选增强项，不作为当前事实描述。
