...
2.  **IM 接口对接 (调查建议)**：
    ...
    -   **同步策略**：当前聊天消息以 REST 接口持久化，并通过 WebSocket(STOMP) 推送刷新；订单候选与 AI 结果仍以 REST 查询/页面事件补偿为主。Webhook 与非聊天业务实时补偿仍是后续演进方向。
...
-   **前端响应（当前）**：聊天使用 REST 持久化 + WebSocket(STOMP) 推送承载“边发边看”；订单候选、商品确认与订单状态仍以 REST 查询/页面事件补偿为主，并在 UI 中明确私聊 / 订单聊天 / Trip 聊天三种上下文边界。
-   **前端响应（后续）**：将订单状态、AI 候选状态纳入非聊天业务实时推送或 MQ 补偿链路。
