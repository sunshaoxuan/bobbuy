...
> 当前配送能力仍停留在**准备 / 导出 / 状态流转**：输出待配送客户地址摘要、联系人、经纬度与 `PENDING_DELIVERY` / `READY_FOR_DELIVERY` 等状态；不接入 Google Maps / Apple Maps / Waze，也不提供实时追踪或重规划。
...
-   **当前落地**：采购端只导出 `businessId`、客户、地址摘要、联系人、电话、`latitude`、`longitude`，作为后续 ARCH-08 真正接图服务的输入。
...
-   **本轮边界**：不调用第三方地图 API，不承诺路径排序结果，只输出后续算法/地图可消费的节点输入。
...
## 5. 当前实现边界
- 已实现：待配送客户列表、地址摘要、CSV 导出、经纬度字段透出、配送准备状态流转。
- 未实现：真实路径算法、地图渲染、实时配送追踪、第三方导航唤起。
