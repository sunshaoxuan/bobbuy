...
### 4.2 对象存储：MinIO
- **用途**：存储原始聊天截图、 कॉस्टको 购物小票原图、商品官方大图。
...
### 5.2 异步消息与任务队列：RabbitMQ / Kafka
- **选型建议**：
  - **RabbitMQ**：建议用于业务任务流程（如订单状态推演、支付回调补单），支持可靠确认机制。
  - **Kafka**：若未来 IM 消息量达到亿级或需要进行大规模 AI 行为日志分析，则考虑引入 Kafka。
- **任务排队**：AI 密集型任务（如 OCR 识别、大批量图像搜索）将通过队列进行异步削峰。
