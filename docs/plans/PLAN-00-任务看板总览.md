...
| [PLAN-58](PLAN-58-P0-服务器输入接入与放行窗口执行提示词.md) | 服务器输入接入与放行窗口执行 | 🔄 执行中 | 40% | 已新增 `scripts/run-server-release-window.ps1` 自动化服务器窗口；agent token 可临时登录生成，当前仅缺 `SSH_TARGET` / `APP_DIR`，未执行 SSH | 全栈团队 |
