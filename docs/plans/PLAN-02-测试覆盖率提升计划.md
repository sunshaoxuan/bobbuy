...
### 1.2 目标
1. **后端覆盖率**: 从 ~15% 提升至 ≥50%（短期）/ ≥70%（长期）
2. **前端覆盖率**: 从 0% 提升至 ≥60%
3. **清偿技术债务**: 修复 AUDIT-01 遗留的 4 个未解决问题
...
- **Phase 1（测试基础设施）**：1 天（2026-01-16）
- **Phase 2（后端单元测试）**：3 天（2026-01-17 ~ 2026-01-19）
- **Phase 3（前端组件测试）**：2 天（2026-01-20 ~ 2026-01-21）
- **Phase 4（集成测试与验收）**：2 天（2026-01-22 ~ 2026-01-23）
...
| 模块 | 最低要求 | 推荐值 | 当前值 | Phase 2 目标 |
|------|---------|--------|--------|-------------|
| 后端 - Line | 50% | 70% | ~15% | 50% |
| 后端 - Branch | 40% | 60% | ~10% | 40% |
| 前端 - Line | 60% | 75% | 0% | 60% |
