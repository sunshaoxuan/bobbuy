...
| 4 | P1 | 认证与权限生产化 | 已完成 JWT 登录、refresh token 会话治理、header auth 生产禁用、WebSocket 鉴权与最小 service token；mTLS、契约测试与更强浏览器端 token 防护仍未完成 | 不适合在缺少后续收口的情况下继续外扩或深拆服务 | JWT + refresh token + gateway/internal service token 稳定，剩余边界风险明确登记 |
...
- **当前补充状态**：WebSocket 鉴权、refresh token 会话治理、最小 service token 与服务壳 smoke test 已完成；mTLS / service mesh、契约测试与更强浏览器端 token 防护继续登记为后续项。
