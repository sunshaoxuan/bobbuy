...
| 4 | P1 | 认证与权限生产化 | 已完成 JWT 登录、header auth 生产禁用与 WebSocket 鉴权；独立服务间鉴权、refresh token 仍未完成 | 不适合在缺少后续收口的情况下继续外扩或深拆服务 | JWT + 角色绑定稳定，服务间鉴权风险明确登记 |
...
- **当前补充状态**：WebSocket 鉴权已完成；独立服务间 service token / mTLS 与 refresh token 继续登记为后续项。
