...
### 2.5 编号连续性铁律 (Linear Integrity)
- **零断层**：章节编号必须严格单向递增（如 2.1, 2.2, 2.3）。严禁出现 2.2 之后接 2.4 或编号重复。
- **物理与逻辑一致性**：文件名中的编号（如 ARCH-07）必须与内容中的标题编号完全同步。禁止物理编号与逻辑内容背离。

## 3. 语言规范

- **正文**: 必须使用 **简体中文**。
- **专业术语**: 保留英文原文 (如 `Widget`, `Endpoint`)，无需强行翻译。
...
- **列表**: 使用 `-` 或 `1.` 进行列表排版。
- **代码块**: 使用反引号 ``` 包裹代码，并指定语言 (如 `pseudo`, `json`)。
...
## 6. 设计元数据规范 (Design Source Specification)

为了支持未来的多数据库架构并保持“设计元即真理 (Design Source as Truth)”原则，设计文档应遵循以下规范：

- **禁止直接使用物理 SQL**：在描述数据模型或结构变更时，严禁直接贴入特定数据库的物理 SQL 语句。
- **使用逻辑 Schema 表达**：应使用 Markdown 表格描述逻辑字段、类型（逻辑类型如 String, GUID, Decimal） and 约束。
...
- [ ] 存放目录正确。
- [ ] 格式清晰，无排版错误。
- [ ] 符合“设计元即真理”原则，不包含直接的物理 SQL。

## 8. 检查复核说明
- 每次修改文档后，必须手动检查上一级索引文档（如 PROD-02）是否需要同步更新链接或描述。

## 9. 发布前终核制度 (Final Check Audit)
所有文档在正式移交、发布或推送到 GitHub 前，必须执行以下“终核”操作：

- **大纲审计**：使用 `view_file_outline` 或全量读取，强制检查全文标题编号是否出现跳过、重复或逻辑倒置（如前文提到的 5 Whys 根因分析）。
- **引用闭环检查**：涉及跨文档引用的变更，必须在变更结束后逐一点击/读取目标链接，验证其“锚点”或“编号”是否依然有效。
- **同步校验**：涉及系统索引（如 README 或 PROD-02）的变更，必须确保索引中的描述与被索引文档的最新版本完全一致。

**[违反后果]**: 凡出现编号混乱、死链接或逻辑目录与内容不符的文档，均视为“不合规工件”，需立即执行 AUDIT 审计。
