...
- 已新增 `docs/reports/REPORT-06-专用环境发版证据与放行判定.md`；其中记录 `main` 分支默认 CI run `25141502571` 成功，但最终放行结论仍为 `NO_GO`。
- 已新增 `docs/reports/REPORT-07-NO-GO阻断项解阻与放行复判.md`；本轮已补自动触发 CodeQL / Maven dependency-check 与 AI 专用环境 evidence workflow，但最终结论仍为 `NO_GO`。
- `.github/workflows/codeql.yml` 已支持 `push` / `pull_request` / `workflow_dispatch`；run <https://github.com/sunshaoxuan/bobbuy/actions/runs/25142273258> 当前为 `action_required`（0 jobs，需仓库管理员批准/放行）。
- `.github/workflows/dependency-check.yml` 已新增；run <https://github.com/sunshaoxuan/bobbuy/actions/runs/25142273277> 当前为 `action_required`（0 jobs，需仓库管理员批准/放行）。
...
当前代码的后端测试、前端单测、前端构建与 Playwright smoke 已恢复为稳定默认/手动门禁；sample 脚本别名漂移已修复并有本地 dry-run 证明；数据库空库初始化与 PostgreSQL 恢复演练已验证；本轮又补齐了 CodeQL / Maven / AI evidence 的自动化执行路径；但 `REPORT-07` 已明确指出 GitHub Actions 审批/权限、真实 AI/OCR 专用环境与真实旧库 adoption 证据仍未补齐，当前结论仍为 `NO_GO`。
