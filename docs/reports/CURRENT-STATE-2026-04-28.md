...
**日期**: 2026-04-28（2026-04-29 部署、配置与微服务边界决策增量更新）
...
- 当前 AI/OCR 可观测性以响应 trace + 结构化日志为主，监控告警与长期成功率指标仍待后续阶段补齐。
...
8. 已引入 Flyway 基线 migration；`backend` 提供 migration 源码，`core-service` 负责试运行部署中的 PostgreSQL schema 初始化，旧库 adoption 仍需备份与一次性 baseline 评估。
