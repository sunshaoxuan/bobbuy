...
**日期**: 2026-04-28（2026-04-29 部署、配置、微服务边界、WebSocket 鉴权、最小 service token 与服务壳 smoke 增量更新；2026-04-30 补充专用环境发版证据执行与放行判定、Codex Bridge provider；2026-05-01 补充依赖高危版本覆盖与 Compose 宿主机 jar 打包路径；2026-05-02 补充 LLM fallback 修复、真实 sample gate 失败证据、双角色移动端黑盒走查与上线前 NO_GO 复判）
...
- 已更新 `docs/reports/REPORT-07-NO-GO阻断项解阻与放行复判.md`；当前已确认默认 CI、CodeQL high、dependency-check critical/high、Compose 基础健康均已解阻，但真实 AI sample / `e2e:ai` / 旧库 adoption 仍阻断。
...
- 已在本机拉起真实 Compose 栈执行基础健康证据；`Dockerfile.service` Maven PKIX、Nacos cgroup v2 / `ProcessorMetrics`、`nacos-init` CRLF、OCR `/health` 与 gateway health 均已解阻。真实 AI sample gate 已打到接口但 `0 PASS / 3 SCAN_FAIL`，证据归档见 `docs/reports/evidence/ai-onboarding-real-sample-2026-05-02.*`；日志显示当前 `BOBBUY_AI_LLM_MAIN_URL=http://ccnode.briconbric.com:22545` 请求失败且 bridge URL / key 未注入，因此 `AI_RECOGNITION` 仍返回空响应。`e2e:ai` 本轮未作为放行证据重跑；仓库内仍未提供真实旧库 dump。
...
当前代码的后端测试、前端单测、前端构建、服务镜像构建与 Docker/Compose 基础门禁已恢复为稳定基线；sample 脚本别名漂移、Windows PowerShell 报告输出、AuthToken 注入、服务镜像构建脚本、Nacos 初始化脚本、`e2e:ai` Windows 启动脚本、Ollama 空响应 fallback、OpenAI-compatible content-part 解析与 Compose/Nacos Codex Bridge 配置入口均已修复；CodeQL high 与 Maven dependency-check critical/high 已清零；Compose 基础健康已在本机临时 secret 下通过；mock 数据下双角色移动端黑盒走查已通过。当前结论仍为 `NO_GO`，原因收敛为真实 AI/OCR sample gate 失败、真实 AI E2E 未形成通过证据、真实旧库 adoption / restore drill 输入缺失，以及双角色移动端黑盒尚未在真实试运行栈复验。
...
1. 注入可用的 `BOBBUY_AI_LLM_CODEX_BRIDGE_URL` / `BOBBUY_AI_LLM_CODEX_BRIDGE_API_KEY`，或修复 `BOBBUY_AI_LLM_MAIN_URL` 指向的 Ollama/兼容服务，使真实 sample gate 从 `0 PASS / 3 SCAN_FAIL` 收敛为字段级通过。
2. sample gate 通过后，再执行 `RUN_AI_VISION_E2E=1 npm run e2e:ai` 或 `.github/workflows/ai-release-evidence.yml`，并归档 Playwright artifact。
3. 获取脱敏旧库 dump / 历史 schema dump，完成 restore、Flyway adoption/baseline/migrate/validate 与恢复演练。
4. 持续保持默认 CI、CodeQL、dependency-check、Compose health 与服务镜像构建证据可追溯；后续文档更新必须引用本文件或更新本文件，避免 Release/Plan 漂移。
