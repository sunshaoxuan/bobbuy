...
- 当前源码已把高危依赖解析版本提升到 `tomcat-embed-core 10.1.54`、`netty-transport 4.1.132.Final`、`commons-fileupload 1.6.0`；新的 GitHub-hosted dependency-check 复扫 run `25215061203` 已成功，artifact `6749713633` 可下载，摘要降至 `0 critical / 1 high / 10 medium`，唯一 high 为 `postgresql-42.6.2.jar` / `CVE-2026-42198`；本地复扫继续受 `www.cisa.gov` DNS 不可达阻塞。
...
当前代码的后端测试、前端单测、前端构建与 Docker build 默认门禁已恢复为稳定基线；sample 脚本别名漂移已修复并有本地 dry-run 证明；数据库空库初始化与 PostgreSQL 恢复演练已验证；CodeQL 3 个 high 告警已在 main 上标记 fixed，Maven dependency-check artifact 可下载且已记录严重级别摘要；当前分支已完成高危依赖版本覆盖并解除 Compose 服务镜像的 Maven PKIX 阻塞，新 dependency-check 复扫降至 `0 critical / 1 high / 10 medium`；但 pgjdbc high、Nacos cgroup v2 / `ProcessorMetrics` 启动异常、真实 AI/OCR 专用环境与真实旧库 adoption 证据仍未补齐，当前结论仍为 `NO_GO`。
