...
**日期**: 2026-04-28（2026-04-29 部署、配置、微服务边界、WebSocket 鉴权、最小 service token 与服务壳 smoke 增量更新）
...
- `pwsh -NoProfile -Command "& '/home/runner/work/bobbuy/bobbuy/scripts/verify-ai-onboarding-samples.ps1' -MockScanResponsePath '/home/runner/work/bobbuy/bobbuy/docs/fixtures/ai-onboarding-sample-scan-mock.json' -SampleIds @('IMG_1484.jpg','IMG_1638.jpg') -IncludeNeedsHumanGolden"`：通过（已验证 `basePrice -> price` 别名与 optional path 规范化）
- `cd frontend && npm audit --json`：发现 `3 critical / 10 high / 4 moderate`
...
- 已新增 AI sample golden 基线 `docs/fixtures/ai-onboarding-sample-golden.json`、对比脚本 `scripts/verify-ai-onboarding-samples.ps1`、mock dry-run fixture `docs/fixtures/ai-onboarding-sample-scan-mock.json` 与专项报告 `docs/reports/REPORT-03-AI商品字段识别样例验证报告.md`；其中 `basePrice -> price` 字段别名与 optional path 漂移已修复。
- 发版候选证据包见 `docs/reports/REPORT-04-发版候选门禁验收报告.md`；其中已记录真实 AI/OCR、`e2e:ai`、CodeQL、Maven 审计与真实旧库 adoption 的剩余阻断项。
- 安全扫描与依赖审计尚未固化到默认 CI；本次 `npm audit` 已发现 `3 critical / 10 high / 4 moderate`，仍需在发版前完成处置结论。
...
当前代码的后端测试、前端单测、前端构建与 Playwright smoke 已恢复为稳定默认/手动门禁；sample 脚本别名漂移已修复并有本地 dry-run 证明；数据库空库初始化与 PostgreSQL 恢复演练已验证；上线判断仍需要同时参考专用环境门禁与风险登记项，当前不能跳过 `REPORT-04` 中的阻断项。
