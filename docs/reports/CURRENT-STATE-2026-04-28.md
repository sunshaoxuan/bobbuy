...
- 共享 PostgreSQL schema 的 migration 由 `core-service` 独占执行；其余服务通过 Nacos 保持 `spring.flyway.enabled=false`，避免并发迁移。
- Gateway 路由目标：
...
6. 当前单体后端已切换为用户名/密码登录 + HMAC JWT access token；header auth 仅在 dev/test 显式开关开启时保留兼容。
...
4. 为旧库 adoption、回滚评估与数据修复脚本补专项方案。
