...
- `.github/workflows/dependency-check.yml` 已新增；最新 `main` run <https://github.com/sunshaoxuan/bobbuy/actions/runs/25198280108> 已成功，artifact `dependency-check-report`（id `6744112430`）已验证同时包含 HTML/JSON，摘要为 `8 critical / 21 high / 19 moderate`（unique CVE）。
...
当前代码的后端测试、前端单测、前端构建与 Docker build 默认门禁已恢复为稳定基线；sample 脚本别名漂移已修复并有本地 dry-run 证明；数据库空库初始化与 PostgreSQL 恢复演练已验证；CodeQL 3 个 high 告警已在 main 上标记 fixed，Maven dependency-check artifact 可下载且已记录严重级别摘要；但 dependency-check 的 `8 critical / 21 high / 19 moderate`、真实 AI/OCR 专用环境与真实旧库 adoption 证据仍未补齐，当前结论仍为 `NO_GO`。
