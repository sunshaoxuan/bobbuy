...
- 已补手动 CodeQL workflow：`.github/workflows/codeql.yml`；仓库管理员需在 GitHub Actions 中执行并归档结果。
- 前端依赖审计已清零 `critical/high`，当前剩余 `6 moderate` 均为 Vite/Vitest dev-only 风险，需按 `REPORT-05` 的豁免与升级计划继续跟踪。
