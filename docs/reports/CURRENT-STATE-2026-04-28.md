...
5. 当前正式内部试运行仍等待 PLAN-58 服务器输入预检查与窗口复跑；缺少 `SSH_TARGET` 与 `APP_DIR` 时不得宣称服务器已放行。
