...
**日期**: 2026-04-28（2026-04-29 部署、配置、微服务边界、WebSocket 鉴权、最小 service token 与服务壳 smoke 增量更新；2026-04-30 补充专用环境发版证据执行与放行判定、Codex Bridge provider）
...
8. 已补齐 refresh token 会话生命周期治理与浏览器端硬化：登录/刷新通过 `Set-Cookie` 下发 `bobbuy_refresh_token` HttpOnly SameSite cookie（默认 `Path=/api/auth`，生产建议 `Secure=true`），服务端只保存 refresh token hash；默认 access token TTL 为 `BOBBUY_SECURITY_JWT_TTL_SECONDS`（默认 3600 秒），refresh token TTL 为 `BOBBUY_SECURITY_REFRESH_TOKEN_TTL_SECONDS`（默认 604800 秒 / 7 天）。前端只在 localStorage 保存 access token、用户信息与过期时间，不再保存 refresh token 明文；`/api/auth/refresh`、`/api/auth/logout` 采用 `bobbuy_csrf_token` + `X-BOBBUY-CSRF-TOKEN` 的 double-submit CSRF 防护；同一旧 refresh token 并发刷新只允许一次成功，其他请求返回 401。
...
- 已更新 `docs/reports/REPORT-07-NO-GO阻断项解阻与放行复判.md`；当前已确认默认 CI 恢复、CodeQL 最新矩阵成功、Maven dependency-check 在 main 持续执行，但最终结论仍为 `NO_GO`。
- `.github/workflows/codeql.yml` 当前已恢复 `push` / `pull_request` / `workflow_dispatch`；最新成功 run 为 <https://github.com/sunshaoxuan/bobbuy/actions/runs/25177727147>，但恢复后的验证 run <https://github.com/sunshaoxuan/bobbuy/actions/runs/25178669741> 仍为 `action_required`（0 jobs），默认分支 push 分析与 code scanning alert 数仍待下一轮归档。
- `.github/workflows/dependency-check.yml` 已新增；最新 `main` run <https://github.com/sunshaoxuan/bobbuy/actions/runs/25177731775> 当前仍为 `in_progress`，尚未产出 HTML/JSON artifact。
- 前端依赖审计已清零 `critical/high`，当前剩余 `6 moderate` 均为 Vite/Vitest dev-only 风险，需按 `REPORT-05` / `REPORT-06` 的豁免与升级计划继续跟踪。
- Maven OWASP Dependency-Check 于 `2026-04-30` 本地再次尝试执行，仍受 `www.cisa.gov` DNS 解析失败阻塞；仓库内已补 GitHub-hosted workflow，但当前仍受 Actions 审批/权限阻塞，尚未生成可信 HTML/JSON 报告。
...
当前代码的后端测试、前端单测、前端构建与 Docker build 默认门禁已恢复为稳定基线；sample 脚本别名漂移已修复并有本地 dry-run 证明；数据库空库初始化与 PostgreSQL 恢复演练已验证；本轮继续确认 CodeQL 最新矩阵成功，并将 CodeQL `push` 触发补回到 workflow；但 `REPORT-07` 已明确指出 Maven dependency-check artifact、真实 AI/OCR 专用环境与真实旧库 adoption 证据仍未补齐，当前结论仍为 `NO_GO`。
