...
**日期**: 2026-04-28（2026-04-29 部署、配置、微服务边界、WebSocket 鉴权、最小 service token 与服务壳 smoke 增量更新；2026-04-30 补充专用环境发版证据执行与放行判定、Codex Bridge provider；2026-05-01 补充依赖高危版本覆盖与 Compose 宿主机 jar 打包路径；2026-05-02 补充 LLM fallback 修复、真实 sample gate 与真实 e2e:ai PASS 证据、mock 与真实栈双角色移动端黑盒走查、空库上线口径与试运行放行复判；2026-05-03 补充 PLAN-58 服务器输入预检查与放行窗口门禁；2026-05-04 补充本机 WSL 放行窗口 PASS 与 PLAN-00 全部关闭）
...
5. 当前内部/小范围试运行已通过 PLAN-58 本机 WSL 放行窗口；外部 Linux 服务器上线时复用同一脚本以 SSH 模式复跑。
