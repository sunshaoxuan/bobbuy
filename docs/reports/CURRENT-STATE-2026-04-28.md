...
8. 已补齐 refresh token 会话生命周期治理：登录返回 access token + opaque refresh token，服务端只保存 refresh token hash；默认 access token TTL 为 `BOBBUY_SECURITY_JWT_TTL_SECONDS`（默认 3600 秒），refresh token TTL 为 `BOBBUY_SECURITY_REFRESH_TOKEN_TTL_SECONDS`（默认 604800 秒 / 7 天）。前端当前继续用 localStorage 保存 access/refresh token 与过期时间，以控制试运行改动面；更强的 HttpOnly SameSite cookie 防护仍待后续阶段。
...
- OAuth/SSO、更强浏览器端 token 防护、mTLS / service mesh 仍未纳入当前交付范围，需在后续安全任务中继续跟进。
...
1. 为 OAuth/SSO、更强浏览器端 token 防护、mTLS / service mesh、服务间契约测试与更细粒度聊天授权补齐后续方案。
