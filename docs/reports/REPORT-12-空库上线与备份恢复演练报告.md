...
本轮代码侧已完成空库真实栈与双角色黑盒证据；备份恢复演练应在试运行部署窗口用实际服务器卷与 bucket 复跑并补充 artifact。PLAN-56 已建立服务器窗口执行提示词；当前缺少 `SSH_TARGET` 与 `APP_DIR`，服务器恢复演练尚未执行，见 `REPORT-13`。
...
- 提供 `SSH_TARGET` 与 `APP_DIR` 后执行 PLAN-56，才能把本报告从本地验收口径提升为服务器试运行放行证据。
