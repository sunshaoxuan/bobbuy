...
> 2026-04-29 更新：默认上线门禁以 `.github/workflows/ci.yml` 为准。后端 `mvn test`、前端 `npm test`、前端 `npm run build` 已恢复为默认门禁；Flyway PostgreSQL migration 验证、Playwright、AI 真实视觉链路与安全扫描按分层策略执行。
