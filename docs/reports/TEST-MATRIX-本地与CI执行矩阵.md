...
| PLAN-57 服务器放行复判门禁 | 见 `docs/plans/PLAN-57-P0-关闭剩余执行中任务与服务器放行复判提示词.md` | SSH 到 Linux 服务器手工执行 | 否 | 需要提供 `SSH_TARGET`、`APP_DIR` 与服务器 `.env`；通过后才能将 REPORT-07 改为 `GO_INTERNAL_TRIAL` 并关闭 PLAN-24/54/55/56/57/CURRENT |
